package com.Entitys.Escort.Action;
...
public class EscortHandler {
	...
	public FullHttpResponse dowork(ChannelHandlerContext ctx, String[] path, Session session, HttpMethod method,
		...
		System.out.println("in EscortHandler");
		if (loginInfo == null)
	...
	protected FullHttpResponse OnCreate(ChannelHandlerContext ctx, Session session, HttpMethod method,
		...
		Map<String, String> newparams = Utils.paramswritefilter(params, escort, role, corporationsid);
		Utils.UpdateFromMap(newparams, escort);
		escort.createtime(operatesid);
		Long sid = null;
		try {
			sid = escortDaoImpl.insert(escort, operatesid);
		} catch (Exception e) {
			tx.rollback();
			JSONObject jsonObject = new JSONObject();
			jsonObject.put("status", 4);
...
//		String pswDB = MD5Utils.MD5(username, password, salt);
		...
		newlogininfo.setType(LoginInfo.TYPE_ESCORT);
		...
		jsonObject.put("msg", "押运员信息创建成功,请记下账号密码，防止遗失");
	...
	protected FullHttpResponse OnUpdate(ChannelHandlerContext ctx, Session session, HttpMethod method,
		...
		Long updatesid = null;
		if (operateusertype == LoginInfo.TYPE_ESCORT) {// 本公司分配员工无权修改信息
			corporationsid = loginInfo.getCorporationsid();
		...
		// 获取输入并验证
		Escort escort = null;
		LoginInfo escortLoginInfo = null;
		try {
			List<QueryBean> escortQueryBean = new ArrayList<QueryBean>();
			escortQueryBean.add(new QueryBean(Escort.class.getName(), "escort", "corporationsid", corporationsid, QueryBean.TYPE_EQUAL));
			escortQueryBean.add(new QueryBean(Escort.class.getName(), "escort", "sid", updatesid, QueryBean.TYPE_EQUAL));
			List<Escort> escorts = escortDaoImpl.getListBy(escortQueryBean, true);
			if (escorts.size() == 0) {
				JSONObject jsonObject = new JSONObject();
				jsonObject.put("status", 4);
				jsonObject.put("msg", "查无此押运员信息");
				jsonObject.put("content", "");
				return NettyUtils.getResponse(iskeepAlive, jsonObject.toString());
			}
			if (escorts.size() > 1) {
				JSONObject jsonObject = new JSONObject();
				jsonObject.put("status", 4);
				jsonObject.put("msg", "数据库出错，押运员主键重复");
				jsonObject.put("content", "");
				return NettyUtils.getResponse(iskeepAlive, jsonObject.toString());
			}
			escort = escorts.get(0);
			escortLoginInfo = session
					.createQuery(
							"from LoginInfo where datastatus = " + Bean.CREATED + " and usersid = " + updatesid
									+ " and type = " + LoginInfo.TYPE_ESCORT + " and corporationsid = " + corporationsid,
							LoginInfo.class)
					.uniqueResult();
			if (escortLoginInfo == null) {
				JSONObject jsonObject = new JSONObject();
				jsonObject.put("status", 4);
				jsonObject.put("msg", "查无此押运员信息");
				jsonObject.put("content", "");
...
//		}
		...
		Transaction tx = session.beginTransaction();
		// 更新登录信息
		String oldlogininfo = Utils.getJsonObject(escortLoginInfo).toString();
		if(username != null){
			escortLoginInfo.setUsername(username);
		}
		if(password != null){
			String salt = Utils.getRandomString(10);
			String pswDB = MD5Utils.MD5(password, salt);
			escortLoginInfo.setPassword(pswDB);
			escortLoginInfo.setSalt(salt);
		}
...
//			String salt = Utils.getRandomString(10);
//			String pswDB = MD5Utils.MD5(objectloginInfo.getUsername(), newpassword, salt);
//			objectloginInfo.setPassword(pswDB);
//			objectloginInfo.setSalt(salt);
//		}
//		if(newpassword != null && username != null){
...
//		}
		if (phone != null)
			escortLoginInfo.setPhone(phone);
		escortLoginInfo.updatetime(operatesid);
		try {
			ValidUtils.ValidationWithExp(escortLoginInfo);
		} catch (Exception e) {
			JSONObject jsonObject = new JSONObject();
			jsonObject.put("status", 4);
			jsonObject.put("msg", e.getMessage());
		...
		MySession.OnUpdate(oldlogininfo, escortLoginInfo, session, operatesid);
		// 更新基础信息
		String oldobject = Utils.getJsonObject(escort).toString();
		if (username != null)
			params.put("username", username);
		if (phone != null)
			params.put("phone", phone);
		...
		try {
			escortDaoImpl.update(escort, oldobject, operatesid);
		} catch (Exception e) {
			tx.rollback();
			JSONObject jsonObject = new JSONObject();
			jsonObject.put("status", 4);
		...
		MySession.OnUpdate(oldobject, escort, session, operatesid);
		tx.commit();
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("status", 0);
		jsonObject.put("msg", "押运员信息更新成功");
		jsonObject.put("content", "");
		return NettyUtils.getResponse(iskeepAlive, jsonObject.toString());
	}
	...
	protected FullHttpResponse OnDelete(ChannelHandlerContext ctx, Session session, HttpMethod method,
		...
		// 删除关联信息
		{
			if (escortLoginInfo.getOnlinetype() != null
					&& escortLoginInfo.getOnlinetype() == LoginInfo.ONLINETYPE_WEB
					&& escortLoginInfo.getToken() != null)
				WebSocketUtils.RemoveCtx(escortLoginInfo.getToken());
			// 暂时不删除统计信息
...
//			}
		...
		try {
			escortDaoImpl.delete(escort, operatesid);
		} catch (Exception e) {
			JSONObject jsonObject = new JSONObject();
			jsonObject.put("status", 4);
			jsonObject.put("msg", e.getMessage());
		...
		MySession.OnDelete(escortLoginInfo, session, operatesid);
		tx.commit();
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("status", 0);
		jsonObject.put("msg", "押运员信息删除成功");
		jsonObject.put("content", "");
		return NettyUtils.getResponse(iskeepAlive, jsonObject.toString());
	}
	...
	protected FullHttpResponse OnQuery(ChannelHandlerContext ctx, Session session, HttpMethod method,
		...
		// 角色解析，所属公司sid确定（只能修改所属公司的信息，当然super可以指定任意公司，政府没有修改权限）
		if (operateusertype == LoginInfo.TYPE_ESCORT) {// 本公司分配员工只能查询自己的信息
			role = LoginableBasicInfo.ROLE_USER;
			corporationsid = loginInfo.getCorporationsid();
		...
		while(iterator.hasNext()){
			...
			queryList.add(new QueryBean(Escort.class.getName(), "escort", field, value, valuetype));
		}
		...
		List<Escort> escorts = null;
		Long total = null;
		try {
			escorts = escortDaoImpl.getListBy(queryList, true, intlimit, intpage);
			total = escortDaoImpl.selectCount(queryList, true);
		} catch (Exception e) {
		...
		for (int i = 0; i < escorts.size(); i++) {
			JSONObject tmpjsonObject = Utils.getJsonObject(escorts.get(i));
