package com.Entitys.Driver.Action;
...
public class DriverHandler {
	...
	public FullHttpResponse dowork(ChannelHandlerContext ctx, String[] path, Session session, HttpMethod method,
		...
		System.out.println("in DriverHandler");
		if (loginInfo == null)
	...
	protected FullHttpResponse OnCreate(ChannelHandlerContext ctx, Session session, HttpMethod method,
		...
		Map<String, String> newparams = Utils.paramswritefilter(params, driver, role, corporationsid);
		Utils.UpdateFromMap(newparams, driver);
		driver.createtime(operatesid);
		Long sid = null;
		try {
			sid = driverDaoImpl.insert(driver, operatesid);
		} catch (Exception e) {
			tx.rollback();
			JSONObject jsonObject = new JSONObject();
			jsonObject.put("status", 4);
...
//		String pswDB = MD5Utils.MD5(username, password, salt);
		...
		newlogininfo.setType(LoginInfo.TYPE_DRIVER);
		...
		jsonObject.put("msg", "驾驶员信息创建成功,请记下账号密码，防止遗失");
	...
	protected FullHttpResponse OnUpdate(ChannelHandlerContext ctx, Session session, HttpMethod method,
		...
		Long updatesid = null;
		if (operateusertype == LoginInfo.TYPE_DRIVER) {// 本公司分配员工无权修改信息
			corporationsid = loginInfo.getCorporationsid();
		...
		// 获取输入并验证
		Driver driver = null;
		LoginInfo driverLoginInfo = null;
		try {
			List<QueryBean> driverQueryBean = new ArrayList<QueryBean>();
			driverQueryBean.add(new QueryBean(Driver.class.getName(), "driver", "corporationsid", corporationsid, QueryBean.TYPE_EQUAL));
			driverQueryBean.add(new QueryBean(Driver.class.getName(), "driver", "sid", updatesid, QueryBean.TYPE_EQUAL));
			List<Driver> drivers = driverDaoImpl.getListBy(driverQueryBean, true);
			if (drivers.size() == 0) {
				JSONObject jsonObject = new JSONObject();
				jsonObject.put("status", 4);
				jsonObject.put("msg", "查无此驾驶员信息");
				jsonObject.put("content", "");
				return NettyUtils.getResponse(iskeepAlive, jsonObject.toString());
			}
			if (drivers.size() > 1) {
				JSONObject jsonObject = new JSONObject();
				jsonObject.put("status", 4);
				jsonObject.put("msg", "数据库出错，驾驶员主键重复");
				jsonObject.put("content", "");
				return NettyUtils.getResponse(iskeepAlive, jsonObject.toString());
			}
			driver = drivers.get(0);
			driverLoginInfo = session
					.createQuery(
							"from LoginInfo where datastatus = " + Bean.CREATED + " and usersid = " + updatesid
									+ " and type = " + LoginInfo.TYPE_DRIVER + " and corporationsid = " + corporationsid,
							LoginInfo.class)
					.uniqueResult();
...
//		}
		...
		Transaction tx = session.beginTransaction();
		// 更新登录信息
		String oldlogininfo = Utils.getJsonObject(driverLoginInfo).toString();
		if(username != null){
			driverLoginInfo.setUsername(username);
		}
		if(password != null){
			String salt = Utils.getRandomString(10);
			String pswDB = MD5Utils.MD5(password, salt);
			driverLoginInfo.setPassword(pswDB);
			driverLoginInfo.setSalt(salt);
		}
...
//		}
		if (phone != null)
			driverLoginInfo.setPhone(phone);
		driverLoginInfo.updatetime(operatesid);
		try {
			ValidUtils.ValidationWithExp(driverLoginInfo);
		} catch (Exception e) {
			JSONObject jsonObject = new JSONObject();
			jsonObject.put("status", 4);
			jsonObject.put("msg", e.getMessage());
		...
		MySession.OnUpdate(oldlogininfo, driverLoginInfo, session, operatesid);
		// 更新基础信息
		String oldobject = Utils.getJsonObject(driver).toString();
		if (username != null)
			params.put("username", username);
		if (phone != null)
			params.put("phone", phone);
		...
		try {
			driverDaoImpl.update(driver, oldobject, operatesid);
		} catch (Exception e) {
			tx.rollback();
			JSONObject jsonObject = new JSONObject();
			jsonObject.put("status", 4);
		...
		MySession.OnUpdate(oldobject, driver, session, operatesid);
		tx.commit();
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("status", 0);
		jsonObject.put("msg", "驾驶员信息更新成功");
		jsonObject.put("content", "");
		return NettyUtils.getResponse(iskeepAlive, jsonObject.toString());
	}
	...
	protected FullHttpResponse OnDelete(ChannelHandlerContext ctx, Session session, HttpMethod method,
		...
		// 删除关联信息
		{
			if (driverLoginInfo.getOnlinetype() != null
					&& driverLoginInfo.getOnlinetype() == LoginInfo.ONLINETYPE_WEB
					&& driverLoginInfo.getToken() != null)
				WebSocketUtils.RemoveCtx(driverLoginInfo.getToken());
			// 暂时不删除统计信息
...
//			}
		...
		try {
			driverDaoImpl.delete(driver, operatesid);
		} catch (Exception e) {
			JSONObject jsonObject = new JSONObject();
			jsonObject.put("status", 4);
			jsonObject.put("msg", e.getMessage());
		...
		MySession.OnDelete(driverLoginInfo, session, operatesid);
		tx.commit();
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("status", 0);
		jsonObject.put("msg", "驾驶员信息删除成功");
		jsonObject.put("content", "");
		return NettyUtils.getResponse(iskeepAlive, jsonObject.toString());
	}
	...
	protected FullHttpResponse OnQuery(ChannelHandlerContext ctx, Session session, HttpMethod method,
		...
		while(iterator.hasNext()){
			...
			queryList.add(new QueryBean(Driver.class.getName(), "driver", field, value, valuetype));
		}
		...
		Long total = null;
		try {
			drivers = driverDaoImpl.getListBy(queryList, true, intlimit, intpage);
			total = driverDaoImpl.selectCount(queryList, true);
		} catch (Exception e) {
		...
		for (int i = 0; i < drivers.size(); i++) {
			JSONObject tmpjsonObject = Utils.getJsonObject(drivers.get(i));
